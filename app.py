...
# 4. 공식 현재 순위 데이터 (26라운드 종료 기준 팀별 누적 성적)
...
def fetch_kleague_official_standings():
    ...
    return pd.DataFrame(default_teams), "🟢 공식 현재 순위 및 잔여 경기 직접 수정 모드 활성화"

# 5. 1~26라운드 실제 기록 및 27라운드 이후 잔여 경기 리스트
@st.cache_data(ttl=60)
def fetch_past_and_future_matches():
    RAW_MATCHES = [
        ...
        {"R": 1, "홈팀": "부산 아이파크", "homeScore": 2, "원정팀": "김해 FC 2008", "awayScore": 0},
    ...
    # 27~34라운드 잔여 경기 샘플 (사용자가 결과를 바꿀 수 있음)
    remaining_matches = [
        {"R": 27, "홈팀": "대구 FC", "원정팀": "수원 삼성 블루윙즈"},
        {"R": 27, "홈팀": "부산 아이파크", "원정팀": "서울 이랜드 FC"},
        {"R": 27, "홈팀": "전남 드래곤즈", "원정팀": "성남 FC"},
        {"R": 28, "홈팀": "수원 삼성 블루윙즈", "원정팀": "수원 FC"},
        {"R": 28, "홈팀": "대구 FC", "원정팀": "경남 FC"},
    ]
...
st.info("공식 순위 및 1~26라운드 실제 결과를 반영하고, 잔여 경기 결과를 직접 입력하여 승격 확률을 예측합니다.")
st.caption(f"{status_msg}")
st.divider()

def run_simulation_with_user_inputs(df_base, rem_matches, user_results, sim_count):
    df = df_base.copy()
    teams = df['팀'].values
    n_teams = len(teams)
    ...
    # 사용자가 직접 입력한 잔여 경기 결과 먼저 누적 반영
    for idx, match in enumerate(rem_matches):
        res_type = user_results.get(idx, "무승부 (1:1)")
        h_team = match["홈팀"]
        a_team = match["원정팀"]
        
        if h_team not in team_idx or a_team not in team_idx:
            continue
        ...
        if "홈승" in res_type:
            base_pts[h_i] += 3
            base_gf[h_i] += 2; base_ga[h_i] += 0
            base_gf[a_i] += 0; base_ga[a_i] += 2
        elif "원정승" in res_type:
            base_pts[a_i] += 3
            base_gf[h_i] += 0; base_ga[h_i] += 2
            base_gf[a_i] += 2; base_ga[a_i] += 0
        else: # 무승부
            base_pts[h_i] += 1
            base_pts[a_i] += 1
            base_gf[h_i] += 1; base_ga[h_i] += 1
            base_gf[a_i] += 1; base_ga[a_i] += 1

    # 몬테카를로 시뮬레이션용 배열 생성 (사용자가 입력하지 않은 나머지 무작위 경기들 추정용)
    pts_sim = np.tile(base_pts, (sim_count, 1))
    gf_sim = np.tile(base_gf, (sim_count, 1))
    ga_sim = np.tile(base_ga, (sim_count, 1))
    
    total_games = 34
    for i in range(n_teams):
        rem = total_games - games_played[i]
        if rem > 0:
            sim_adds = np.random.choice([3, 1, 0], size=(sim_count, int(rem)), p=[0.38, 0.26, 0.36]).sum(axis=1)
            pts_sim[:, i] += sim_adds

    gd_sim = gf_sim - ga_sim
    rank_matrix = np.zeros((sim_count, n_teams))
...
with col1:
    st.subheader("⚙️ 모델 설정 및 잔여 경기 결과 입력")
    sim_count = st.slider("시뮬레이션 횟수", 1000, 20000, 5000, step=1000)
    
    st.divider()
    st.subheader("📝 잔여 경기 직접 승패 입력")
    st.caption("아래 잔여 경기들의 결과를 선택하면 즉시 시뮬레이션에 반영됩니다.")
    
    user_results = {}
    for idx, match in enumerate(remaining_matches):
        h_logo = get_logo_html(match['홈팀'], size=18)
        a_logo = get_logo_html(match['원정팀'], size=18)
        label = f"R{match['R']} | {match['홈팀']} vs {match['원정팀']}"
        
        res = st.selectbox(
            label,
            options=["홈승 (2:0)", "무승부 (1:1)", "원정승 (0:2)"],
            index=1,
            key=f"rem_match_{idx}"
        )
        user_results[idx] = res

    st.divider()
    st.subheader("🔄 1~26라운드 실제 경기 기록 조회")
        ...
        for m in r_matches:
            ...
            <div class="match-card">
                <span style="font-size: 0.95rem; font-weight: bold;">
                    {h_logo} {m['홈팀']} 
                    <span style="color:#0085FF; margin: 0 6px;">{m['실제홈득점']} : {m['실제원정득점']}</span> 
                    {a_logo} {m['원정팀']}
                </span>
            </div>
            """
...
with col2:
    st.subheader("📊 팀별 승격 확률 및 순위 예측")
    
    rank_matrix, teams, team_idx = run_simulation_with_user_inputs(
        df_standings, remaining_matches, user_results, sim_count=sim_count
    )
    ...
    fig = px.bar(
        ...
        y="빈도수", 
        text="빈도수", 
        title=f"<b>{target_team} 최종 순위 분포 예측 ({sim_count:,}회 시뮬레이션)</b>",
